...
struct t_OptionShortCut
{
	int		iShortCutView;
};

struct t_OptionToolTip
{
	int		iToolTipView;
};
...
const	int		DEFAULT_BGM_VOLUME		= 3;/// -1000 : OptionDlg::Create()참고
const	int     DEFAULT_EFFECT_VOLUME	= 5;
...
const	int		g_iDefaultAntiAliasing	= 0;
...
//#define CAMERA_FAR_FOG(I)					g_TblCamera.m_ppVALUE[ I ][ 7 ].GetINT()
	...
	UINT GetVideoAntiAliasing(){ return m_VideoOption.iAntiAliasing; }
	...
	void SetVideoAntiAliasing(UINT iMode){ m_VideoOption.iAntiAliasing = iMode; }
	...
	UINT GetSoundOff(){ return m_SoundOption.iSoundOff; }
	...
	/// ShortCut View
	void SetShortCutOption( t_OptionShortCut & option );
	void GetShortCutOption( t_OptionShortCut & option );

	/// ToolTip View
	void SetToolTipOption( t_OptionToolTip & option );
	void GetToolTipOption( t_OptionToolTip & option );
	...
	void  SetQuickBarExtDlgType( int iType ){ m_iQuickBarExtDlgType = iType; }
	...
	void	ApplyAntiAliasingOption( short i);
...
protected:
	...
	t_OptionToolTip  m_ToolTipOption;

	bool			m_bShowPartyMemberHpGuage;
	int				m_iQuickBarDlgType;     /// Vertical, Horizontal
	int				m_iQuickBarExtDlgType;     /// Vertical, Horizontal
