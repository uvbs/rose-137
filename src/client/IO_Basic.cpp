...
{	
	...
	///	for pet bones..
	///
	//06. 12. 11 - 김주현 : 카트 종류 확장.카트 - 21~29, 캐슬기어 - 31~39 특정 카트/캐슬기어의 본을 교체할 경우 여기를 수정.
	for(int i = PET_TYPE_CART01; i < 30; i++)
	{
		m_SkelPET[ i ] = m_SKELETON.Add_SKELETON ( "3DDATA\\Pat\\Cart\\Cart01.zmd" );
	}

	for(int j = PET_TYPE_CASTLE_GEAR01; j < 40; j++)
	{
		m_SkelPET[ j ] = m_SKELETON.Add_SKELETON ( "3DDATA\\Pat\\CastleGear\\CastleGear02\\CastleGear02.ZMD" );
	}
	for(int k = PET_TYPE_NEW_RIDE01; k < 50; k++)//this is for our new pat/pet? I think we could also use some of cart or castle gear
	{
		m_SkelPET[ k ] = m_SKELETON.Add_SKELETON ( "3DDATA\\Pat\\CastleGear\\CastleGear02\\CastleGear02.ZMD" );//( "3DDATA\\Pat\\NewRide\\NewRide.ZMD" );
	}
