/*
	$Header: /Client/Network/SendPACKET.h 8     07-03-20 10:35a Raning $
*/
...
public   :
	...
	void Send_cli_LOGIN_REQ2( DWORD dwSeq, char * szSessionKey );
...
//	void Send_cli_CLICK_HOTICON (BYTE btListIDX);
	...
	bool Send_cli_GET_FIELDITEM_REQ( WORD iServerObject );
	...
	void Send_cli_SKILL_LEVELUP_REQ (WORD wSkillSLOT, short nNextLevelSkillIDX=0);

	void Send_cli_SELF_SKILL (WORD wSkillSLOT);
	void Send_cli_TARGET_SKILL (int iClientTarget, WORD wSkillSLOT);
	void Send_cli_POSITION_SKILL (D3DVECTOR &PosTO, WORD wSkillSLOT);
	...
	///제조 관련
	void Send_cli_CREATE_ITEM_REQ( WORD wSkillSLOT, char cTargetItemTYPE, short nTargetItemNO, short* pnUseItemINV );
//	void Send_cli_CREATE_ITEM_EXP_REQ();
	...
	void Send_cli_MESSENGER_CHAT( DWORD dwUserTag, const char * toName, char* pszMsg );
#else
	...
	//소켓 뚫기...
	void Send_cli_CRAFT_DRILL_SOCKET_REQ( BYTE btType, short nUseInvSLOTorNpcIDX, BYTE btTargetInvIDX );
	...
	// -- 클랜전 관련
	void Send_cli_CLANWAR_JOIN_ACK( gsv_cli_CLAN_WAR_OK *p );
	void Send_cli_CLANWAR_INFO( int value );
	...
	void Send_cli_DUMMY_1();

	
#ifdef __EXPERIENCE_ITEM
	void Send_cli_MILEAGE_ITEM_TIME(DWORD dwIndex);
#endif
