/*
	$Header: /Client/Sound/IO_Sound.cpp 24    05-12-01 10:30a Raning $
*/
#include "stdAFX.h"
#include "IO_Sound.h"
...
bool CSoundLIST::KEY_PlaySound (t_HASHKEY HashKEY)
{
	...
	if ( NULL == pSoundData ) {
		// The sound file is not registered in the sound files list
		return false;
	}

#ifdef USE_DEFAULT_3D_SOUND
	if (!m_SOUND.Set3D (pSoundData->m_pSoundData, true)) return false; // 3d mode no set
#endif
...
#ifdef USE_DEFAULT_3D_SOUND
	//if (!m_SOUND.Set3D (pSoundData->m_pSoundData, false)) return false; // Set 3D mode
...
#ifdef USE_DEFAULT_3D_SOUND
	if (!m_SOUND.Set3D (pSoundData->m_pSoundData, true)) return false; // 3D mode not set
#endif
...
//-------------------------------------------------------------------------------------------------
/// @bug pSndFileDATA there.. pDATA->m_DATA NULL  work in some case?
...
void CSoundLIST::SubSoundFile (t_HASHKEY uiKEY)
{
	...
	if ( pSndFileDATA ) 
	{
		if( pSndFileDATA->m_DATA )
		{
			...
			{
				pSndFileDATA->m_iRefCnt = 0;	// Set to 1 to force the deletion to be registered in the waiting buffer .... 
...
void CSoundLIST::SetVolume ( int iSoundVolume )
{
	m_iSoundVol = iSoundVolume;	
}
