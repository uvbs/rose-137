...
//---------------------------------------------------------------------------------
void CObjCHAR::ActionSpecialEVENT(int iIndex)
{
	int iEffectIDX = iIndex;
	if ( iEffectIDX ) 
	{
		CEffect *TargetEFT = g_pEffectLIST->Add_EffectWithIDX( iEffectIDX, true );
		if ( TargetEFT ) 
		{
			TargetEFT->LinkNODE ( this->GetZMODEL() );
			TargetEFT->SetParentCHAR( this );
			AddExternalEffect( TargetEFT );
			TargetEFT->InsertToScene ();
		}
	}
}
...
void CObjCHAR::ShowCastingEffect( int iActionIDX )
{
	switch( iActionIDX )
	{
		...
		case 74:
			{
				//홍근 : 2,3 사용 안하기로 함.   // 조성현 2006 0327 사운드 중첩
...
void CObjCHAR::ActionSkillCastingFrame( int iCastingOrder )
{
	...
	/// 타격 사운드 설정			
	if(iCastingOrder == 2 || iCastingOrder == 3)  // 조성현 2006 0327 사운드 중첩
		return;
