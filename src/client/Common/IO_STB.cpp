...
bool STBDATA::Load2 (char *szFileName, bool bHasTYPE, bool bMakeKEY, int VFSMode)
{
	SetSTBName(szFileName);
	ClientLog(LOG_DEBUG,"IO_STB::Load2. opening STB file %s",szFileName);

	CGameSTB cFILE;
	int nX, nY, nFindKEY;
	char *pStr;

	if ( cFILE.Open (szFileName, VFSMode) ) 
	{
		this->m_bLoad2 = true;
