/*
	$Header: /Client/Common/CObjAI.h 138   06-12-29 12:21a Choo0219 $
*/
#ifndef	__CCOMMAND_H
#define	__CCOMMAND_H
#include "CGameOBJ.h"
...
enum
{
	...
	COMMON_COMMAND_CASTLEGEAR	= 14,	// 캐슬기어 탑승 스킬.
	
	//reserve
	COMMON_COMMAND_15			= 15,
	COMMON_COMMAND_16			= 16,
	COMMON_COMMAND_17			= 17,
	COMMON_COMMAND_18			= 18,
};
...
#define RECOVER_STATE_CHECK_TIME_EVO    2500	/// 2.5초마다 회복(_NoRecover 일땐 서버에서 보내주는값을 사용하므로 이시간은 업데이트 시간.)
...
public:
			...
			void		Set_AdjustSPEED( float fSpeed );
...
//--------------------------------------------------------------------------------
/// NPC  충돌에 대한 이펙트 처리
    bool				ProcSpecialMotionFrame (void);
	bool                ProcOneSpecialActionFrame(int iIndex );
	virtual void		ActionSpecialEVENT (int iIndex)=0{ *(int*)0 = 10; };
//--------------------------------------------------------------------------------
