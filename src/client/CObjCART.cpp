...
//			ruPos+=((ruDir * -1.0f) * 200.0f);    //2006 11월21일 충돌 문제로 삭세..
...
//박지호::
	...
	if ( 0 == nFileIDX ) 
	{
		...
		char Temp[512];
#ifdef _GBC
		sprintf(Temp,"INVALID PET MOTION : WeaponIDX = %d, ActionIdx = %d",this->m_nWeaponIDX, nActionIdx );
#else
		sprintf(Temp,"INVALID PET MOTION : AbilIDX = %d, ActionIdx = %d ", this->m_nAbilIDX, nActionIdx);
#endif
		g_itMGR.OpenMsgBox( Temp );
...
void CObjCART::SetCMD_ATTACK (int iServerTarget, WORD wSrvDIST, const D3DVECTOR& PosGOTO)
{
	...
	//----------------------------------------------------------------------------------------------------
#if defined(_GBC)
	//박지호::운전자 및 2인승한 유저를 앉히는 모션을 설정한다.
	//카트를 탄 상태에서 아바타는 어떠한 행동도 취하지 않는다. 
	m_pObjParent->Set_MOTION( this->GetRideAniPos() + PETMODE_AVATAR_ANI_STOP1 );
