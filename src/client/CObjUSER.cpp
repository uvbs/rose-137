...
void CObjUSER::Init_DefaultValue (void)
{
	...
	Set_STR( 100 );		// Muscular strength
	Set_DEX( 100 );		// Agility
	Set_INT( 10 );		// Intellect
	Set_CON( 10 );		// Concentration
	Set_CHARM( 10 );	// Charm
	Set_SENSE( 10 );	// feeling

	tagITEM sITEM;
	::ZeroMemory( &sITEM, sizeof(tagITEM) );

	sITEM.m_cType   = ITEM_TYPE_ARMOR;			// Armor
	sITEM.m_nItemNo = 72;	
	Set_EquipITEM ( EQUIP_IDX_ARMOR,		sITEM );

	sITEM.m_cType   = ITEM_TYPE_KNAPSACK;		// Non-appearance
	sITEM.m_nItemNo = 0;	
	Set_EquipITEM ( EQUIP_IDX_KNAPSACK,		sITEM );	

	sITEM.m_cType   = ITEM_TYPE_GAUNTLET;		// Gloves
	sITEM.m_nItemNo = 71;	
	Set_EquipITEM ( EQUIP_IDX_GAUNTLET,		sITEM );

	sITEM.m_cType   = ITEM_TYPE_BOOTS;			// Shoes
	sITEM.m_nItemNo = 44;	
	Set_EquipITEM ( EQUIP_IDX_BOOTS,		sITEM );

//*
	sITEM.m_cType   = ITEM_TYPE_WEAPON;			// Right hand
...
//	209 ;	// Longbow
	...
	sITEM.m_nItemNo = 0;	// Left-handed
	Set_EquipITEM ( EQUIP_IDX_WEAPON_L,		sITEM );
//*/

	this->SetPartITEM( BODY_PART_FACE,		1 );	// Face
	this->SetPartITEM( BODY_PART_HAIR,		0 );	// Basic hair
...
void CObjUSER::RideCartToggle( bool bRide )
{
	...
	/// Pat Depending on whether the board allows changing the activation / deactivation of skill.
	/// 
...
//-------------------------------------------------------------------------------------------------
/// Using the avatar information is set in the class, and updates.
...
/*override*/int	CObjUSER::Get_BulletNO()
{
	...
	/// magic bullet consumption weapons
...
/// What is the current pet is dehydrated combination?
...
//--------------------------------------------------------------------------------
/// @brief  : CInventoryDlg And it updates the Virtual Inventory and WeightRate.
//--------------------------------------------------------------------------------
void	CObjUSER::UpdateInventory()
{
	...
	/// Weight ratio renewal	
	int iMaxWeight  = Get_MaxWEIGHT();///The maximum amount Possession

	//assert( iMaxWeight > 0 );
	if( iMaxWeight <= 0 )
		return;

	int iCurrWeight = 0;///Current carrying capacity
	int iItemWeight = 0;///Weight per item
	...
	///A certain percentage (80%, 100%) came over and down to notify the server.
...
//-------------------------------------------------------------------------------------------------
/// Method for a message related to processing stored in the server
...
/// Summon the total amount available.
...
//*----------------------------------------------------------------------
/// @brief overriden CObjAVT::GetStamina() -- CObjAVT::Proc For ObjUSER Called when calling from.
//*----------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------
/// @brief Functions that save additional stats cart
///			1. Cart Gauge
///		- Because it lowers the value in the server deflate this part 2005/7/27 - nAvy
//------------------------------------------------------------------------------------------------------
...
BOOL CObjUSER::SetCoolTimeUseItem(char* uName,short iItem)
{
	...
	else
	{
		...
		//Set the index of the item itself, and delay ticks.
...
//------------------------------------------------------------------------------------------------------
///Set the bakjiho: Skill cooldown.
//------------------------------------------------------------------------------------------------------
BOOL CObjUSER::SetCoolTimeSkill(char* uName,int iSkill)
{
	...
	//Set blank when alone.
	else
	{	
		//If you are dilrek own self ....
		...
		//Skill sets its own index and the delay tick.
...
//---------------------------------------------------------------------------
/// Cho, Sung - Hyun 2005 12-27 get the camera direction
//---------------------------------------------------------------------------
