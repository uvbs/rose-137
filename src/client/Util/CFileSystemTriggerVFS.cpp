...
bool CFileSystemTriggerVFS::OpenFile( const char* fname, int iMode )
{
	...
	pFileHandle->btEncrypted=0;
	//if(fp_->btFileType==0){
	size_t spt;
	spt = pFileHandle->sFileName.find_last_of("/\\");
	std::string hashstring1 = pFileHandle->sFileName.substr(spt+1, (pFileHandle->sFileName.length()-spt));
	...
	vfseek(pFileHandle, 0, VFSEEK_END);
	long off_set = vftell(pFileHandle);
	if(off_set>16)
	{
		off_set-=16;
		vfseek(pFileHandle,off_set, VFSEEK_SET);
		char buffer[16];
		vfread(&buffer,1,16,pFileHandle);
															...
															if(buffer[13]==crypttable[13]){
																if(buffer[14]==crypttable[14]){
																	if(buffer[15]==crypttable[15]){
																		pFileHandle->btEncrypted=0x90;
																		pFileHandle->lEndOff-=16;
	...
	vfseek(pFileHandle,0, VFSEEK_SET);
	if( m_pData != NULL )
	{
		delete m_pData;
		m_pData = NULL;
...
//	return ::vfread( lpBuf, 1, nCount, m_pFileHandle );
		/*uint32*/int read_size = 0;
	if (m_pFileHandle) {
		#define KEYSIZE 0x1000
		long Mpointer = vftell(m_pFileHandle) % KEYSIZE;
		read_size = vfread(lpBuf, 1, nCount, m_pFileHandle);
		if(m_pFileHandle->btEncrypted==0x90)
		{
			//MessageBox(0,fp_->sFileName.c_str(),"decrypting",0);
			size_t spt;
			spt = m_pFileHandle->sFileName.find_last_of("/\\");
			std::string hashstring1 = m_pFileHandle->sFileName.substr(spt+1, (m_pFileHandle->sFileName.length()-spt));
			//OutputDebugString(hashstring1.c_str());
			unsigned long key = StrToHashKey(hashstring1.c_str());
			...
			{
				*((char*)lpBuf+k)^= crypttable[Mpointer];
				Mpointer++; Mpointer %= KEYSIZE;
			}
			//maybe delete [] crypttable;
		}
	}
	else {
		return 0;
