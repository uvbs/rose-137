...
t_sounddata* CD3DSOUND::CreateSoundData (t_wavedata *pWave, DWORD dwCreationFlags, GUID guid3DAlgorithm)
{
	...
	if ( pSound ) {
		...
		m_dsBufferParams.flMaxDistance = SOUND_DEFAULT_MAX_DISTANCE; // This applies only to the volume
		m_dsBufferParams.flMinDistance = SOUND_DEFAULT_MIN_DISTANCE; //  This applies until the maximum volume
...
bool CD3DSOUND::UpdateListener ( const CCamera * pCamera )
{
	...
	// Setting the position of the listener position avatar
