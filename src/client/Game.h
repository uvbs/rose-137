/*
	$Header: /Client/Game.h 105   07-03-20 10:27a Raning $
*/
#ifndef	__GAME_H
#define	__GAME_H
...
public:	
	...
	int					m_iLangIDX;		// Raven- Language Index
	...
	SYSTEMTIME			m_SystemTime;			//시스템 타임.
	int					m_dwServerTimeGap;		//서버에서 준 시간과 클라이언트의 시간 차이값
												//시간 차이 보정값 = 서버시간 - 클라이언트 시간;
												//서버 현재 시간 = 클라이언트 시간 + 시간 차이 보정값;
	
	time_t				m_ServerGMTime;			//
	...
	bool				m_SkipIntroNote;
	bool				m_SkipIntroLogo;
	...
	void DrawFontCameraPosition(); //조성현 2005 10 - 20 카메라 상태 출력..
