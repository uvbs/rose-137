...
public:
	...
	enum eMAKESOCKET_TYPE{
		MAKESOCKET_NONE,
		MAKESOCKET_NPC,  /// NPC에 의한 소켓 생성(05.10.29-김주현 : 현재 기획안에는 없다)
		MAKESOCKET_ITEM, /// 아이템에 의한 소켓 생성.
	};
...
//	int		GetCurrCursorType(){ return m_iCurrentCursorType; }
	...
	void	SetUsingRepairItemInvenIdx( int iItemInvenIdx );				// 수리 아이템의 인벤토리 인덱스 번호 지정
	int		GetUsingRepairItemInvenIdx();									// 수리 아이템의 인벤토리 인덱스 가져오기.
	
	void	SetRepairNpcSvrIdx( WORD wNpcSvrIdx );
	WORD    GetRepairNpcSvrIdx();

	void	SetAppraisalNpcSvrIdx( WORD wNpcSvrIdx );						// 감정 npc의 서버 인덱스 번호 지정.
	WORD	GetAppraisalNpcSvrIdx();										// 감정 npc의 서버 인덱스 가져오기.


	void	SetUsingMakeSocketItemInvenIdx( int iItemInvenIdx );			// 소켓 생성 아이템의 인벤토리 인덱스 번호 지정
	int		GetUsingMakeSocketItemInvenIdx();								// 소켓 생성 아이템의 인벤토리 인덱스 가져오기.
	//----------------------------------------------------------------------------------------------
	/// 마우스 타겟지점 이펙트
	//----------------------------------------------------------------------------------------------
