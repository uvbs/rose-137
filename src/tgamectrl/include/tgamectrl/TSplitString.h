...
public:
	CTSplitString(void);
	CTSplitString( int iFont, const char* pszStr, int iDisplayWidth ,unsigned uiCodePage );
	~CTSplitString(void);

	int	GetLineCount();
	const char* GetString( int iIndex );

	bool Split( int iFont, const char* pszStr, int iDisplayWidth ,unsigned uiCodePage );
	void Clear();
	
	SIZE & GetSizeText( int iFont, const char * szStr );
	SIZE GetColmnRow(int iPos);	//
