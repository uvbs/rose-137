...
public:
	...
	~CSinglelineString(void);
	void	set_string( const char* msg, const RECT& rect );				// 문자열
	void	set_string( const char* msg, const RECT& rect , int ifont );	// 문자열
	void	set_rect( const RECT& rect );									// 범위
	void	set_color( DWORD color );										// 색상
	void	set_format( DWORD format );										// 정렬
	void	set_font( int iFont );											// 폰트
