...
CTDialog* CTDialog::m_pFocusDlg = NULL;
...
unsigned int CTDialog::Process( UINT uiMsg,WPARAM wParam,LPARAM lParam )
{
	...
	//홍근 : 오전 10:09 2006-11-07 : Dlg에서 return사용 세팅.
	switch( uiMsg )
	{
	case WM_KEYDOWN:
		{
...
{
	//__AUTO_LOGIN 에서 창이 안보일 경우에도 종료 커맨드 실행돼야 하기 때문에 처음으로 올렸다.
	...
	if( m_pCaption )
	{
		...
		if(m_pCaption->IsClicked())
		{
			...
			//int w = CTControlMgr::GetInstance()->GetDrawMgr()->GetScreenWidth();
			//int h = CTControlMgr::GetInstance()->GetDrawMgr()->GetScreenHeight();

			////Dlg가 화면 밖으로 나가는것 방지.
			//if( ptNew.x < 0 || ptNew.y < 0 
			//	|| ptNew.x+GetWidth() > CTControlMgr::GetInstance()->GetDrawMgr()->GetScreenWidth()
			//	|| ptNew.y+GetHeight() > CTControlMgr::GetInstance()->GetDrawMgr()->GetScreenHeight() )
			//{					
			//	int i = 0;
			//	i++;
			//}
...
void CTDialog::MoveWindowCenter()
{
	MoveWindow( (CTControlMgr::GetInstance()->GetDrawMgr()->GetScreenWidth()-GetWidth())/2,
				(CTControlMgr::GetInstance()->GetDrawMgr()->GetScreenHeight()-GetHeight())/2 );
}
...
void CTDialog::SetReturnID(int id)
{
	m_iReturnID = id;
}

int CTDialog::GetReturnID()
{
	return m_iReturnID;
}
