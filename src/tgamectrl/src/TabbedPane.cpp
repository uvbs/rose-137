...
CWinCtrl* CTabbedPane::Find( const char * szName )
{
	CWinCtrl * pCtrl = NULL;
	std::list< S_TabbedPane >::iterator iter;
	for( iter = m_Tabs.begin(); iter != m_Tabs.end(); ++iter)
	{
		if( strcmp( iter->m_pRadioButton->GetControlName(), szName ) == 0)
		{
			return iter->m_pRadioButton;
		}
		if(pCtrl = iter->m_pContainer->Find( szName ) )
		{
			return pCtrl;
		}		
	}
...
CWinCtrl* CTabbedPane::Find( int iID )
{
	CWinCtrl * pCtrl = NULL;
	std::list< S_TabbedPane >::iterator iter;
	for( iter = m_Tabs.begin(); iter != m_Tabs.end(); ++iter)
	{
		if( iter->m_pRadioButton->GetControlID() == iID )
		{
			return iter->m_pRadioButton;
		}
		if(pCtrl = iter->m_pContainer->Find( iID ) )
		{
			return pCtrl;
		}		
	}
