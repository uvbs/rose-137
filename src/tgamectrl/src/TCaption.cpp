...
void CTCaption::Draw()
{
	...
	if( !m_strTitle.empty() )
	{
		...
		rc.right	= GetWidth();
		rc.bottom	= GetHeight();
		pFontMgr->Draw( m_iFont, true, &rc, m_dwColor,  DT_VCENTER|DT_LEFT, m_strTitle.c_str() );
	}
	...
	if( !m_strTitle.empty() )
	{
		...
		pFontMgr->Draw( 0, true, 0, 0, D3DCOLOR_ARGB(255,255,255,255), m_strTitle.c_str() );
	}
...
void CTCaption::SetString( const char* pszString, int iFont)
{
	m_strTitle = pszString;
	m_iFont = iFont;
}

void CTCaption::SetStringColor( DWORD dwColor )
{
	m_dwColor = dwColor;
}
